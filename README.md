...
- **セクター資金流入の可視化** — どこに資金が集まっているかをヒートマップとランキングで表示。**大分類（11セクター）⇄ 細分類（業種）** を切り替え可能
- **バリュー株の自動ピックアップ** — 低PER・低PBR・高配当・高ROE の条件で割安・優良株を自動抽出し、バリュースコア順に表示
- **個別銘柄ページ** — 株価推移（6か月チャート）と主要指標。ヒートマップ/スクリーナーのタイル・行から遷移
- **⭐ ウォッチリスト** — 気になる銘柄を保存（ブラウザのlocalStorageに保存・バックエンド不要）
- **米国株・日本株 両対応** — 全市場 / 🇺🇸米国 / 🇯🇵日本 を切り替え可能
- **データキャッシュ** — ライブ取得を5分キャッシュし高速化・API負荷を軽減
...
- セクター資金フローの**時系列推移**（セクターETFの履歴を用いた時系列グラフ）
- 銘柄ユニバースのさらなる拡張（S&P500・TOPIX構成銘柄の自動読み込み＋DBキャッシュ）
- ポートフォリオ管理・アラート通知
